...
@pytest.fixture()
def passlayer_ingress_proxy_http(passlayer_ingress_proxy):
    return UrlRequester(
        passlayer_ingress_proxy.dockertester.base_url_for_container(
            passlayer_ingress_proxy.container_id, 8079
        )
    )


@pytest.fixture()
def fakepasslayer(dockertest: DockerTester):
    environment = {}
    dumped = False
    container_id = dockertest.launch_container(
        container_registry() + "/fakepasslayer",
        "fakepasslayer",
        image_tag=image_tag(),
        environment=environment,
        ports=[80, 81],
    ...
    try:
        yield dockertest.wait_for_web_service(container_id, 80, "health")
...
class RabbitMqBroadcaster(TestContainerMixin):
    def __init__(self, hostname, port, exchange_name):
    ...
    def progress(self, instance_name, jobid, progress=0.0, message=""):
        self._publish(instance_name, "progress", jobid, f"{progress},{message}")

    def signal(self, instance_name, jobid, state, message):
        if message:
            body = f"{state},{message}"

        else:
            body = state

        self._publish(instance_name, "signal", jobid, body)

    def warning(self, instance_name, jobid, warning_message):
        self._publish(instance_name, "warning", jobid, warning_message)

    def info(self, instance_name, jobid, info_message):
        self._publish(instance_name, "info", jobid, info_message)

    def proxy_update_required(self, instance_name):
        self._publish(instance_name, "update", "proxy", "Proxy update required")

    def proxy_upgrade_signal(self, instance_name):
        self._publish(
            instance_name, "upgrade", "proxy", "Proxy update for TAS upgrade start"
        )

    def cf_kv_store_update(self, instance_name):
        self._publish(
            instance_name,
            "update",
            "cf_kv_updater",
            "Cloudflare KV store update required",
        )

    def cf_kv_store_update_all(self):
        self._publish(
            None,
            "update_all",
            "cf_kv_updater",
            "Cloudflare KV store bulk update required",
        )

    def add_maintenance(
        self, instance_name, expiration_ttl=870, maintenance_type="maintenance"
    ):
        ...
        self._publish(
            ...
            "add_maintenance",
            "cf_kv_updater",
            json.dumps(maintenance_parameters),
        )

    def remove_maintenance(self, instance_name):
        self._publish(
            instance_name,
            "remove_maintenance",
            "cf_kv_updater",
            "Remove redirect to maintenance page after upgrade",
        )

    def monitoring_configuration_update_required(self, instance_name, location):
        self._publish(
            instance_name,
            "update",
            "monitoring",
            f"Monitoring configuration update required,{location}",
        )

    def schedule_downtime(self, instance_name, location, duration, requester, comment):
        self._publish(
            instance_name,
            "schedule",
            "downtime",
            "Schedule monitoring downtime,{location},{duration},{requester},{comment}",
        )
    ...
    def _ensure_channel(self):
        if self._amqp_channel is None:
            ...
            self._amqp_channel.exchange_declare(
                exchange=self.exchange_name, type="topic", auto_delete=False
            )

    def _publish(self, instance_name, tail, jobid, body):
        self._ensure_channel()
        routing_key = f"{instance_name}.{jobid}.{tail}"
        content = amqp.basic_message.Message(body)
...
@pytest.fixture()
def rabbitmq(dockertest: DockerTester):
    environment = {}
    dumped = False
    container_id = dockertest.launch_container(
        "rabbitmq",
        "rabbitmq",
        image_tag="3.11.5-management-alpine",
        environment=environment,
        ports=[5672, 15672],
        force_pull=True,
    )
        ...
        while time.time() < (start_time + 20):
            ...
            try:
                (
                    management_ip,
                    management_port,
                ) = dockertest.get_connectable_host_and_port(container_id, 15672)
                ...
                amqp_ip, amqp_port = dockertest.get_connectable_host_and_port(
                    container_id, 5672
                )
                if amqp_ip is None:
                ...
                broadcaster = RabbitMqBroadcaster(
                    hostname=amqp_ip, port=amqp_port, exchange_name="saas"
                )
    ...
    try:
        ...
        connection._dockertest = dockertest
        connection._container_id = container_id
        yield connection
